...
MODULE_AUTHOR("Venura Perera, Gaurav Setia");
MODULE_LICENSE("GPL");
...
asmlinkage long my_exit_group(struct pt_regs reg)
{
	...
	del_pid(current->pid); 	//Deleting current PID from all lists
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// if syscall in ax is being monitored by current process
	// or .monitored for ax = 2 (all pids are monitored)
	// then log message using params in pt_regs registers
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	//the syscall must be valid (not negative, not > NR_syscalls-1, and not MY_CUSTOM_SYSCALL itself -> 0)
	if(syscall <= 0 || syscall > NR_syscalls - 1){
		return -EINVAL;
	}

	// PID must be valid for last two commands
	...
	// First two commands must be root
	// Assuming current_uid() returns 0 if root
	...
	// Make sure last two commands are root and are not monitoring all ids or 
	// the pid is the same pid as the calling process (owner)
	if(cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING){
	...
	//Make sure the system call is being intercepted before being de-intercepted
	if(REQUEST_SYSCALL_RELEASE == cmd && table[syscall].intercepted == 0){
		spin_unlock(&my_table_lock);
		return -EINVAL;
	}

	//can only stop monitoring a specific pid if there the system call is set to monitoring a certain list of ids (monitored == 1)
	if(REQUEST_STOP_MONITORING == cmd && pid != 0 && table[syscall].monitored != 1){
		spin_unlock(&my_table_lock);
		return -EINVAL;
	}

	//Make sure the system call isn't already being intercepted before trying to intercept
	if(REQUEST_SYSCALL_INTERCEPT == cmd && table[syscall].intercepted == 1){
		spin_unlock(&my_table_lock);
		return -EBUSY;
	}

	//cannot monitor a pid that is already being monitored
	if(REQUEST_START_MONITORING == cmd && pid != 0 && table[syscall].monitored == 1){
		spin_unlock(&my_table_lock);
		return -EBUSY;
	}
	...
	// printk(KERN_DEBUG "Hello world3!\n");
	if (cmd == REQUEST_SYSCALL_INTERCEPT){
		
		spin_lock(&my_table_lock);


		//keep track this this syscall is being intercepted 
		table[syscall].intercepted = 1;
		//store system call in table at .f
		...
		//Release this system call from being intercepted
		...
		if(pid == 0){
			// set this system call to be monitored for all pid
			table[syscall].monitored = 2;
		}else if (add_pid_sysc(pid, syscall) != 0){

			spin_unlock(&my_table_lock);
			// add_pid_sysc returned an error meaning that there is no more memory
			return -ENOMEM;
			
		}else{
			// set this system call to be monitored for this pid
		...
		if(pid == 0){
			// set this system call to remove all monitoring pids
			destroy_list(syscall);
		}else if (del_pid_sysc(pid, syscall) != 0){

			spin_unlock(&my_table_lock);
			// del_pid_sysc returns an error meaning that there is no pid in this system call
			return -EINVAL;
...
static int init_function(void) {
	// Must declare any variable at the start of the function or the kernel gives warning
	int i; 	// Must declare for loop variable here instead of in for loop before because Kernel doesn't like it

	// lock the system call table because its being edited 
	spin_lock(&sys_call_table_lock);

	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL]; //saving original

	orig_exit_group = sys_call_table[__NR_exit_group]; //DONT KNOW IF __NR_exit_group is the correct variable name

	//set call table to writable
	set_addr_rw((unsigned long)sys_call_table);

	// replace 0th postion of the call table with my_syscall
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;
	// replace the exit group function with our custom exit group function which will call the original exit group function
	sys_call_table[__NR_exit_group] = my_exit_group;

	//set call table to read only
	set_addr_ro((unsigned long)sys_call_table);

	//unlock - done editing sys_call_table
	spin_unlock(&sys_call_table_lock);
	...
	// unlock table - done editing table
	spin_unlock(&my_table_lock);
...
{        
	...
	// Only need to deintercept all syscalls and cleanup all pid lists

	// cleaning up pid lists
	int i;
	for(i = 0; i < NR_syscalls; i++){
	...
	// Lock call table since we are editing call table and permissions
	spin_lock(&sys_call_table_lock);
